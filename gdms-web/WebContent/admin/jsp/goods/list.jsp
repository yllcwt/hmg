...
function updateGtid(gtid, $select){
	...
	alert(goodsid);
	//写一个WEB服务方法
	//在这里写Ajax请求，调用上面的web方法
	//在业务层接口，扩展一个跟新商品类别的方法
	//在业务层实现类实现相应的方法
	//在数据层DAO接口扩展一个跟新商品类别的方法
	//在DAO对应的mapper文件中定义相应的update语句
	//在客户端处理服务端返回的结果，显示消息气泡
	
	$.ajax({
		url : "rest/hongyan/push/json",
		type : "post",
...
function dobaocun(a){
	...
	var data = {"name": $input1.val(), "spec": $input2.val(),
				"price": $input3.val(), "amout": $input4.val(),
				"goodsid":goodsid}
